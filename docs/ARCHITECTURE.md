...
- `render.js`: PC向けDOM描画、参照パネル、設定UI、画像事前読込、手札ID差分による順次ドロー/捨て札移動演出
...
- `mobile/render-sp.js`, `audio-sp.js`, `main-sp.js`: スマホUIに合わせた固有実装。PC版と同じ手札ID差分によるカード移動演出を含む
...
カード移動演出は描画前後のカードID集合を比較し、新しい手札カードへ500ms刻みのCSSアニメーション遅延を付与します。捨て札へ追加されたIDは直前DOMを複製して捨て札位置まで飛ばします。状態更新、通信、保存は演出を待たず、ルールロジックから分離しています。
