...
- `render.js`: PC向けDOM描画、参照パネル、加工交換モーダル、設定UI、画像事前読込、手札ID差分による順次ドロー/捨て札/料理融合演出
...
- `mobile/render-sp.js`, `audio-sp.js`, `main-sp.js`: スマホUIに合わせた固有実装。PC版と同じカード移動・料理融合・加工交換モーダルを含む
...
カード移動演出は描画前後のカードID集合を比較し、新しい手札カードへ自分側500ms、相手側300ms刻みのCSSアニメーション遅延を付与します。捨て札へ追加されたIDは直前DOMを複製して捨て札位置まで飛ばします。料理履歴の追加と材料の捨て札移動が同時に起きた場合は材料を融合演出へまとめ、料理画像へ変換してから捨て札方向へ送ります。状態更新、通信、保存は演出を待たず、ルールロジックから分離しています。
