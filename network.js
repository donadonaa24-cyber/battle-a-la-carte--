(function () {
    ...
    const errors = {
        ...
        INVALID_PASSPHRASE: '参加する場合は、部屋を作った人から聞いた6桁の合言葉を入力してください。',
    ...
    function updateSkillDetails() {
        const details = $('online-skill-details');
        const skill = window.getSkillDefinitionByKey?.($('online-skill')?.value);
        if (!details) return;
        details.replaceChildren();
        if (!skill) {
            details.textContent = 'スキルを選ぶと、ここに発動条件と効果が表示されます。';
            return;
        }
        const rows = [
